...
func sync_new_direction(direction):
	if direction != Vector2.ZERO:
...
func enemy_animations():
	if direction != Vector2.ZERO:
		...
		animation = "walk_" + returned_direction()
		$AnimatedSprite2D.play(animation)
	else:
		animation  = "idle_" + returned_direction()
		$AnimatedSprite2D.play(animation)

# returns the animation direction
func returned_direction():
	# it normalizes the direction vector to make sure it has length 1 
	var normalized_direction  = direction.normalized()
	var default_direction = "side"
	if normalized_direction.y >= 7:
		return "down"
	if normalized_direction.y <= -7:
		return "up"
	if normalized_direction.x >= 7:
...
func _on_animated_sprite_2d_animation_finished():
	...
	if $AnimatedSprite2D.animation.begins_with("attack_"):
		...
		get_tree().root.get_node("/root").add_child(bullet)
	...
	if now >= bullet_fired_time:
		...
		if target != null:
			if target.name == "Player" and player.health > 0:
				# shooting anim
				is_attacking = true
				animation  = "attack_" + returned_direction()
				$AnimatedSprite2D.play(animation)
